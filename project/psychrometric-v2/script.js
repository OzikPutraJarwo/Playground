    ...
    pl.innerHTML = State.points.map((p, i) => `
        <div class="list-item ${p.id === State.selectedPointId ? 'active' : ''}" onclick="selectPoint(${p.id})">
            <div class="item-header">
                ...
                <div class="item-name">T:${p.t.toFixed(1)} / W:${(p.w * 1000).toFixed(1)}</div>
        ...
        <div class="list-item ${z.id === State.selectedZoneId ? 'active' : ''}" onclick="selectZone(${z.id})" style="border-left:4px solid ${z.color}">
            <div class="item-header">
                ...
                <div class="item-actions">
                    <button class="icon-btn" onclick="openZoneSettings(${z.id})">⚙️</button>
...
function openZoneSettings(id) {
    ...
    document.getElementById('zoneColorInput').value = z.color;
    document.getElementById('zoneModal').style.display = "flex";
}
function saveZoneSettings() {
    const id = parseInt(document.getElementById('editZoneId').value);
    const z = State.zones.find(z => z.id === id);
    if (z) { z.name = document.getElementById('zoneNameInput').value; z.color = document.getElementById('zoneColorInput').value; }
    updateLists(); drawChart(); closeModal('zoneModal');
}
function finishZone() {
    let finalPoints = [];
...
// 3. CHART RENDERING
// ==========================================
const margin = { top: 35, right: 60, bottom: 45, left: 60 };
...
function drawChart() {
    ...
    axesLayer.append("text").attr("class", "axis-label").attr("x", w / 2).attr("y", h + 35).text("Dry Bulb Temperature (°C)");
    axesLayer.append("text").attr("class", "axis-label").attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -45).text("Humidity Ratio (kg/kg)");
...
function handleMouseMove(e, x, y, minT, maxT, maxH, Patm) {
    ...
    if (t < minT || t > maxT || w < 0 || w > maxH) { document.getElementById("info-panel").style.display = "none"; return; }
    const d = calculateAllProperties(t, w, Patm);
    const p = document.getElementById("info-panel");
    p.style.display = "block";
    let lp = margin.left + mx + 20; if (lp + 250 > chartWrapper.clientWidth) lp -= 270;
    p.style.left = lp + "px"; p.style.top = (margin.top + my + 20) + "px";
...
function generateHTMLGrid(d) {
    return `
        <div class="detail-row"><span class="det-label">Tdb:</span><span class="det-val">${d.Tdb.toFixed(1)}°C</span></div>
        <div class="detail-row"><span class="det-label">W:</span><span class="det-val">${d.W.toFixed(4)}</span></div>
        <div class="detail-row"><span class="det-label">RH:</span><span class="det-val">${d.RH.toFixed(1)}%</span></div>
        <div class="detail-row"><span class="det-label">Twb:</span><span class="det-val">${d.Twb.toFixed(1)}°C</span></div>
        <div class="detail-row"><span class="det-label">Tdp:</span><span class="det-val">${d.Tdp.toFixed(1)}°C</span></div>
        <div class="detail-row"><span class="det-label">h:</span><span class="det-val">${d.h.toFixed(1)}</span></div>
        <div class="detail-row"><span class="det-label">v:</span><span class="det-val">${d.v.toFixed(3)}</span></div>
        <div class="detail-row"><span class="det-label">ρ:</span><span class="det-val">${d.rho.toFixed(2)}</span></div>
        <div class="detail-row"><span class="det-label">Pw:</span><span class="det-val">${d.Pw.toFixed(0)}</span></div>
        <div class="detail-row"><span class="det-label">Pws:</span><span class="det-val">${d.Pws.toFixed(0)}</span></div>
        <div class="detail-row"><span class="det-label">μ:</span><span class="det-val">${d.mu.toFixed(1)}%</span></div>
        <div class="detail-row"><span class="det-label">Cp:</span><span class="det-val">${d.cp.toFixed(3)}</span></div>
    `;
}
